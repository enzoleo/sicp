...
(defun primes-vector-search (lower upper)
  ...
  (let* ((fix-lower (if (= 0 (mod lower 2)) (+ lower 1) lower))
         (fix-upper (if (= 0 (mod upper 2)) (- upper 1) upper))
         ;; Vector buffer storing continous primes
         (vec-buf (make-array
                   (+ 1 (/ (- fix-upper fix-lower) 2))
                   :initial-element 0))
         (prime-counter 0))
    
    ;; Simple loop to test odd numbers
    ...
    ;; Cut the original vector buffer. Return a sub-vector.
    (subseq vec-buf 0 prime-counter)))
